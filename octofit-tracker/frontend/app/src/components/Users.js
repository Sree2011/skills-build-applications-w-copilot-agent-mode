...
const Users = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setUsers(filtered);
      console.log('Users to display:', filtered);
    } catch (err) {
  ...
  const openDetails = (item) => { setSelected(item); setShowModal(true); console.log('Open user:', item); };
      ...
      <div className="card-body">
        ...
        <div className="table-responsive">
          <table className="table table-striped table-bordered">
            ...
            <tbody>
              {users.map((user, idx) => (
                <tr key={user.id || idx} style={{cursor: 'pointer'}} onClick={() => openDetails(user)}>
